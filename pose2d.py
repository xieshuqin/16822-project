...
sys.path.append('./DEKR/tools')
sys.path.append('./DEKR/lib')
...
def create_2d_pose_model(cfg_name='experiments/coco/inference_demo_coco.yaml'):
    args = Args(cfg_name)
...
def pose_to_bbox(poses, image):
    ...
    for pose in poses:
        ...
        xmin, xmax = max(xcenter - 0.5 * scale * width, 0), min(xcenter + 0.5 * scale * width, w - 1)
        ymin, ymax = max(ycenter - 0.5 * scale * height, 0), min(ycenter + 0.5 * scale * height, h - 1)
