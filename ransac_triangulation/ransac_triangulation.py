...
def ransac_triangulaton(poses_2d, camera_matrices, threshold=10, imgs=None, debug=False):
    # init
    ...
    for combination in all_camera_combinations:
        ...
        if debug:
            ...
            show3Dpose(cur_joints_3Dpts, ax, radius=128, mpii=2)
            plt.savefig('combination_' + str(combination[0]) + ',' + str(combination[1]) + str('_triangulate.jpg'))

        # Back project to the 2D poses
        cur_joints_4Dpts = np.hstack((cur_joints_3Dpts, np.ones((len(cur_joints_3Dpts), 1))))
        for cam_idx in range(len(camera_matrices)):
            ...
            if debug:
                if imgs is None:
                    ...
                    img = np.array(imgs[i].convert('RGB'))[:, :, ::-1]
            ...
            for joint_idx in range(len(cur_joints_4Dpts)):
                ...
                dist = np.linalg.norm(projected_2Dpt[:2] - np.array(
                    poses_2d[cam_idx][joint_idx]))  # the projected point - the pose estimated point
            ...
            # debug
            if debug:
                cv2.imwrite('combination_' + str(combination[0]) + ',' + str(combination[1]) + '_cam' + str(
                    cam_idx) + '_back_projected.jpg', img)
        ...
        if (cur_joint_inliers > best_joint_inliers or (
                cur_joint_inliers == best_joint_inliers and cur_allCam_erros < best_errors)):
