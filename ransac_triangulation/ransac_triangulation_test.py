...
def triangulate_single(u1,P1,u2,P2):
    def make_constraints(m, p):
        return [
            p[1]*m[2]-m[1],
            m[0]-p[0]*m[2]
        ]
    c=make_constraints(P1,u1)+make_constraints(P2,u2)
    u,s,vt=np.linalg.svd(c)
    print(s[-1])
    return (vt[-1]/vt[-1, -1])[:3],None

def triangulate_multi(u1,P1,u2,P2):
    a=[]
    b=[]
    for uu1,uu2 in zip(u1,u2):
        aa,bb=triangulate_single(uu1,P1,uu2,P2)
        a.append(aa)
        b.append(bb)
...
def ransac_triangulaton(poses_2d, camera_matrices, threshold=20):
    ...
    for combination in all_camera_combinations:
        ...
        cur_joints_3Dpts, vis = triangulate_multi(pose1, M1, pose2, M2)
        ...
        for cam_idx in range(len(camera_matrices)):
            ...
            for joint_idx in range(len(cur_joints_4Dpts)):
                ...
                if debug:
                    ...
                    x=poses_2d[cam_idx][joint_idx][0]
                    y=poses_2d[cam_idx][joint_idx][1]
                    img = cv2.circle(np.array(img), (int(x),int(y)) ,2, (255,0,0), 5)
